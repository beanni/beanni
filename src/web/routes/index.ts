    ...
    const balanceHistoryChartData = {
      ...
      datasets: _(balanceData)
        ...
        .map((_value, key) => ({
          ...
          data: _(dates)
            .map((d) => {
              const dataPoint = _(balanceData).find(
                (bd) => bd.label === key && bd.date === d
              );
    ...
    const latestBalances = _(balanceData)
      ...
      .map((_value, key) => {
        ...
        return {
          ...
          valueTypeName:
            ValueType[latestBalance?.valueType ?? ValueType.Unknown],
          label: key,
...
function calculatePerformanceByPeriods(
  ...
  if (minDateRaw == null || maxDateRaw == null) {
    return null;
  }
  const minDate = new Date(
    new Date(minDateRaw).toISOString().substring(0, 10) + " 00:00"
  );
  const maxDate = new Date(
    new Date(maxDateRaw).toISOString().substring(0, 10) + " 00:00"
  );
  ...
  const result = {
    years: _(periodsWithMovements)
      .groupBy((p) => p.year)
      .map((value, key) => {
        const yearStartingBalance = balanceAtDate(
          new Date(`${key}-01-01 00:00`),
          1
        );
        const yearEndingBalance = balanceAtDate(
          new Date(`${key}-12-31 00:00`),
          1
        );
        return {
          year: key,
