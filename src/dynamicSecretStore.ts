...
export class DynamicSecretStore implements ISecretStore {
  ...
  constructor() {
    ...
    if (headless) {
      console.warn(
        "🚨 Found a secret file at " +
          this.headlessSecretPath +
          "; using that for all secrets. Only use this approach in totally headless scenarios."
      );
  ...
  private writeHeadlessSystemHint() {
    ...
    console.info(
      "On headless systems, create an empty file at `" +
        this.headlessSecretPath +
