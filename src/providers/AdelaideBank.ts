...
export class AdelaideBank implements IBankDataProviderInterface {
  ...
  public async login(
    ...
    await page.waitForSelector(".form-group[data-testid='txtUsername']", {
      visible: true,
    });
    ...
    await page.type(
      ".form-group[data-testid='txtPassword'] input[role=textbox]",
      password,
      { delay: 100 }
    );
  ...
  public async getBalances(): Promise<IAccountBalance[]> {
    ...
    for (const row of rows) {
      const accountName = await row.$eval(".account-name", (el) =>
        (el.textContent || "").trim()
      );
      const accountNumber = await row.$eval(".account-number", (el) =>
        (el.textContent || "").trim()
      );

      const balance = await row.$eval(
        ".current-balance .amount-wrapper",
        (el) => {
          const balanceText =
            el.querySelector(".amount")?.textContent?.trim() || "0";
      ...
      const valueType =
        rowClassName.indexOf("account-category-loan") > -1
          ? ValueType["Property Mortgage"]
