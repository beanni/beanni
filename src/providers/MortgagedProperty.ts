...
import { IAccountBalance, IBankDataProviderInterface, ICalculatedProviderInterface, IInstitutionRelationship, ValueType } from "../types";
...
export class MortgagedProperty implements IBankDataProviderInterface, ICalculatedProviderInterface {
    ...
    public async login(
        ...
        config: MortgagedPropertyConfig): Promise<void> {
        this.config = config;
    }
    ...
    public async getCalculatedBalances(otherBalances: IAccountBalance[]): Promise<IAccountBalance[]> {
        if (this.config == null) { throw new Error("Config missing"); }

        const config = this.config;

        const loanBalance = otherBalances.find(bal =>
        ...
        if (loanBalance === undefined) {
            console.log(
                `[${config.provider}] Couldn't find loan balance in this fetch for ${JSON.stringify(config.loan)}. ` +
