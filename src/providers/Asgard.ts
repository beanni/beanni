    ...
    IBankDataHistoricalBalancesProviderInterface
{
  ...
  public async logout(): Promise<void> {
    ...
    await page.goto(
      "https://www.investoronline.info/iol/iollogon/logout_transfer.jsp"
    );
  ...
  public async getBalances(): Promise<IAccountBalance[]> {
    ...
    const tables = await page.$x(
      '//b[contains(text(), "Account type")]/ancestor::table[tbody/tr/td/b]'
    );
    ...
    for (const row of rows) {
      ...
      balances.push({
        ...
        balance: parseFloat(
          cellText[2].trim().replace("$", "").replace(",", "")
        ),
  ...
  public async getHistoricalBalances(
    ...
    await page.goto(
      "https://www.investoronline.info/iol/portfoliovaluation.do"
    );

    for await (const dateToLookup of datesToLookup) {
      console.log(
        `[${this.institution}] Looking up ${dateToLookup
          .toISOString()
          .substring(0, 10)}`
      );

      const dmyFormat = `${dateToLookup.getDate()}/${
        dateToLookup.getMonth() + 1
      }/${dateToLookup.getFullYear()}`;

      await page.focus(
        "form[name=PortfolioValuationForm] input[name=clientEnquiryValuationDate]"
      );
      await page.$eval(
        "form[name=PortfolioValuationForm] input[name=clientEnquiryValuationDate]",
      ...
      const accountName = await this.getTextContentByXPath(
        "//form[@name='PortfolioValuationForm']//table/tbody/tr/td[contains(., 'Account Name')]/following-sibling::td"
      );
      const accountNumber = await this.getTextContentByXPath(
        "//form[@name='PortfolioValuationForm']//table/tbody/tr/td[contains(., 'Account Number')]/following-sibling::td"
      );
      const accountValue = await this.getTextContentByXPath(
        "//form[@name='PortfolioValuationForm']//table/tbody/tr/td[contains(., 'Account Value')]/following-sibling::td"
      );
