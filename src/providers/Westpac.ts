...
export class Westpac
  implements IBankDataProviderInterface, IBankDataDocumentProviderInterface
{
  ...
  public async login(
    ...
    await page.setUserAgent(
      `Mozilla/5.0 (Windows NT 10.0; Win64; x64) ${browserVersion.replace(
        "Headless",
        ""
      )}`
    );
  ...
  public async logout(): Promise<void> {
    ...
    await page.goto(
      "https://banking.westpac.com.au/wbc/banking/handler?TAM_OP=logout"
    );
  ...
  public async getBalances(): Promise<IAccountBalance[]> {
    ...
    await page.goto(
      "https://banking.westpac.com.au/secure/banking/accounts/getsummarystartpage?viewType=Summary"
    );
    await page.waitForSelector(".accounts-summarylistwidget table");

    const accountSummaryRows = await page.$$(
      ".accounts-summarylistwidget table > tbody > tr"
    );
    for (const row of accountSummaryRows) {
      const accountName = await row.$eval(".tf-account-detail a span", (el) =>
        (el.textContent || "").trim()
      );
      ...
      balances.push({
        ...
        balance: parseFloat(
          await row.$eval(".balance.current .balance", (el) =>
            (el.textContent || "")
              .trim()
              .replace("minus", "")
              .replace("$", "")
        ...
        valueType:
          accountNumber.indexOf("xxxx") === 0
            ? ValueType["Consumer Debt"]
  ...
  public async getDocuments(statementFolderPath: string): Promise<void> {
    ...
    await page.goto(
      "https://banking.westpac.com.au/secure/banking/account/statements"
    );
