    ...
    IBankDataHistoricalBalancesProviderInterface
{
  ...
  public async getBalances(): Promise<IAccountBalance[]> {
    ...
    const handleResponse = async (response: puppeteer.HTTPResponse) => {
      ...
      const looksLikeAnAccountResponse =
        /\/api\/adviser\/current\/accounts\/([^/]*?)\?/.test(url);
    ...
    const onResponse = (response: puppeteer.HTTPResponse) => {
      handleResponse(response).then(null, (reason) => {
        throw reason;
      });
    };
    ...
    await page.goto(
      "https://investor.myperpetual.com.au/mozart/investorweb/app/accounts/all-investments"
    );
    await page.waitForNavigation({ waitUntil: "networkidle0" });
  ...
  public async getHistoricalBalances(
    ...
    console.log(
      `[Perpetual] There are ${datesToLookup.length} historical data points to attempt to get`
    );

    const maxHistoricalBatchSize = 30;
    if (datesToLookup.length > maxHistoricalBatchSize) {
      console.log(
        `[Perpetual] Limiting to ${maxHistoricalBatchSize} data points in this batch`
      );
    ...
    await page.waitForNavigation({ waitUntil: "networkidle0" });
    await page.click(
      "adv-investment-summary mat-expansion-panel mat-expansion-panel-header"
    );
    ...
    const handleResponse = async (response: puppeteer.HTTPResponse) => {
      ...
      const regex = new RegExp(
        /\/accounts\/(?<accountNumber>[\w\d]+?)\/parcels?/
      );
      ...
      const balance = _(data)
        .map((f) =>
          f.certDetails.reduce(
            (acc, val) => acc + val.availUnits * val.sellPrice,
            0
          )
        )
        .sum();
    ...
    for await (const dateToLookup of datesToLookup) {
      console.log(
        `[Perpetual] Looking up ${dateToLookup.toISOString().substring(0, 10)}`
      );
      ...
      await page.click(
        "adv-investment-summary mat-expansion-panel adv-as-at-date-select input"
      );
      await page.$eval(
        "adv-investment-summary mat-expansion-panel adv-as-at-date-select input",
      ...
      await page.waitForResponse(
        (response) => response.ok() && response.url().indexOf("/parcels?") > 0,
        { timeout: 20000 }
      );
